    ...
    <div className="max-w-[960px] mx-auto px-4 lg:px-6 py-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200/60 overflow-hidden shadow-sm">
        {notices.map((notice, idx) => (
          <div
            key={notice.id}
            className={`flex items-center justify-between px-5 py-4.5 hover:bg-gray-50/30 cursor-pointer transition-colors ${
              idx < notices.length - 1 ? 'border-b border-gray-100' : ''
            }`}
          >
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <span className="text-[11px] bg-gray-50 text-gray-900 px-2.5 py-1 rounded-lg font-semibold shrink-0">공지</span>
