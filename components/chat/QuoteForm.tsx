...
export default function QuoteForm({ onSubmit, onClose }: QuoteFormProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-5">
      <h3 className="font-bold text-base mb-4">견적 제안</h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="text-sm text-gray-500">
          할인율: <span className="font-bold text-red-500">{discount}%</span>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">발송 예정일 (선택)</label>
          ...
          {!form.instantDelivery && (
            <input
              ...
              className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-primary mt-2"
            />
          )}
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-1">
          <h4 className="font-bold mb-2">거래 요약</h4>
        ...
        <div className="flex gap-3">
          <button type="button" onClick={onClose} className="flex-1 h-10 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">
            취소
          </button>
          <button type="submit" className="flex-1 h-10 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark">
            견적 제안
          </button>
        </div>
      </form>
