...
export default function SellPostItem({ post }: SellPostItemProps) {
  return (
    <Link href={`/board/${post.id}`} className="block group">
      <div className="flex items-center justify-between py-4 px-5 hover:bg-gray-50/40 border-b border-gray-100/80 transition-all duration-200 last:border-b-0">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            <span className="text-sm text-gray-800 font-medium truncate group-hover:text-gray-900 transition-colors">{post.title}</span>
            {post.tags.map((tag) => (
              <span key={tag} className="text-[10px] text-gray-400 shrink-0 hidden md:inline">{tag}</span>
            ))}
            {post.isNew && (
              <span className="text-[9px] bg-gray-900 text-white w-4 h-4 rounded-full flex items-center justify-center shrink-0 font-bold">N</span>
            )}
          </div>
        </div>
        ...
        <div className="text-right shrink-0 ml-4">
          <span className="text-gray-900 font-bold text-sm">{post.discount}%</span>
